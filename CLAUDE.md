...
1. **Session 基準 effort = `high`（固定）**
   - 不整段開 ultracode / xhigh 自動編排。日常主迴圈與 Agent tool 派出的 subagent 跑 `high`（繼承 session）。
   - **Workflow 內的機械葉降 effort**：跑腳本收證據、對照清單逐條驗收、截圖比對這類低推理工作，用 `opts.effort:'low'` 或 `'medium'` 派；architect / reviewer 等判斷葉維持 `high`（Opus 5 低 effort 品質可用，這是省 token 的主控桿）。
...
1. **architect 開發**
   ...
   - **reviewer 全報、主 Claude 過濾**：reviewer 發現的問題全部報出並分級（[嚴重]/[一般]/[建議]），不叫它自我保守；主 Claude 以 [嚴重]+[一般] 作為退修 gate 依據，[建議] 記錄不擋放行
   - **接著呼叫 `reviewer`**
...
- 文件可使用繁體中文
- 寫入磁碟的文件（規格、ADR、知識卡、驗收清單、交接檔、memory）長度對齊實質內容：不補填充章節、重複摘要、樣板段落——這些檔案會吃掉未來 session 的 context
- 註解與程式碼不需要使用 emoji
...
- 自審跑一輪即可，內容照 architect 自審清單（清單載明「查什麼」的領域知識）；不要在清單之外疊加額外的重複驗證輪次——模型本身會自驗，指令疊加只燒 token
- 自審發現功能缺漏、邏輯錯誤、效能瓶頸，直接修正，無需等待使用者指示

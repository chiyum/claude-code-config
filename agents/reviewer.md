...
- **全報、不自我過濾**：發現的問題全部報出來、分級呈現，不要自己判斷「這個可能不值得提」就吞掉——過濾是 orchestrator 的工作（[嚴重]+[一般] 才擋 gate，[建議] 記錄不擋），你的工作是把東西找出來
- **分級要誠實**：nil 風險、競態、資料漏洞、租戶隔離破口 → 一定標 [嚴重]；風格偏好、非必要優化 → 放 [建議]，不要混進需修正清單膨脹退修範圍
